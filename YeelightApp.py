...
def activate_controls():
    ...
    bulb_saturation_sld.config(state='normal', troughcolor='white', fg='black')
    bulb_hue_sld.config(state='normal', troughcolor='white', fg='black')
...
# BG IMAGE
# background_image = tk.PhotoImage(file=r'C:\Users\Martin\Desktop\yl_bg.png')
...
ip_entry = tk.Entry(root, width=20, justify='center')
...
bulb_bright_sld = tk.Scale(root, length=111, from_=0, to=100, state='disabled', fg='gray', orient='horizontal', command=bright_slider_changed)
...
bulb_temp_sld = tk.Scale(root, length=111, from_=1700, to=6500, state='disabled', fg='gray', orient='horizontal', command=temp_slider_changed)
...
bulb_saturation_lbl = tk.Label(root, text="Saturation", wraplength=1, fg='gray')
bulb_saturation_lbl.grid(column=1, row=7, padx=(50, 0), pady=(25, 2))

bulb_saturation_sld = tk.Scale(root, length=160, from_=100, to=0, state='disabled', fg='gray', command=hue_saturation_slider_changed)
bulb_saturation_sld.grid(column=1, row=7, pady=(25, 2), sticky='E')

bulb_hue_lbl = tk.Label(root, text="Hue", fg='gray')
bulb_hue_lbl.grid(column=2, row=9, columnspan=2)

bulb_hue_sld = tk.Scale(root, length=250, from_=0, to=359, state='disabled', fg='gray', orient='horizontal', command=hue_saturation_slider_changed)
...
scene_home_btn.grid(column=1, row=10, pady=30)
...
scene_movie_btn.grid(column=2, row=10)
...
scene_romance_btn.grid(column=3, row=10)
...
scene_candle_btn.grid(column=4, row=10)
...
scene_sunrise_btn.grid(column=1, row=11)
...
scene_sunset_btn.grid(column=2, row=11)
...
scene_rgb_btn.grid(column=3, row=11)
...
scene_police_btn.grid(column=4, row=11)
